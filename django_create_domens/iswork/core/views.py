...
class MainPageView(CreateView):
    ...
    def form_valid(self, form):
        ...
        if web_server == 1:
            result = Domen_Create_Delete().create_domen_apache(domen_name)
        elif web_server ==2:
...
class DomenDeleteView(DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        web_server = self.object.webserver
        if web_server == 1:
            result = Domen_Create_Delete.delete_domen_apache(domen_name)
        elif web_server == 2:
            result = Domen_Create_Delete().delete_domen_nginx(domen_name)
        if result == True:
            self.object.delete()
        return HttpResponseRedirect(self, self.success_url) 
